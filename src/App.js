...
function App() {
  ...
  // Helper: pulisce testo e rende minuscolo
  const cleanText = (txt) => (txt ? txt.toString().toLowerCase().trim() : '');

  // Carica file Excel e restituisce dati JSON del primo foglio
  ...
  // Funzione principale per matching
  const handleMatch = async () => {
    ...
    try {
      ...
      // Filtra righe richiesta con "MOTORE COMPL" o "MOTORE COMPLETO" in colonna RICAMBIO (case insensitive)
      ...
      // Funzione per match "marca modello anno" tra Inventario e Richiesta
      // In Inventario: Marca e Modello sono dentro colonna "Ricambio" (es: "FIAT 500 2007 ...")
      // In Richiesta: Marca, Modello, Anno sono colonne separate (non sempre tutte presenti, ma almeno Anno sì)
      // Implemento una semplice funzione che verifica se marca/modello/anno di richiesta sono "contenuti" nella colonna Ricambio di inventario
      ...
      const matchMarcaModelloAnno = (ricambioInventario, rigaRichiesta) => {
        ...
        // Verifico che almeno marca o modello siano presenti nel ricambio inventario
        // Inoltre, se anno richiesto è presente, cerco che sia almeno contenuto nel testo inventario
        if (!marcaReq && !modelloReq) return false;

        // Check marca o modello almeno una presente nel testo inventario
      ...
      // Ora procedo con il matching:
      // Per ogni motore richiesto, cerco corrispondenza in Inventario su "Veicolo/Tipo Motore (EcoEuro)"
      // e nella riga di inventario colonna "Ricambio" contiene "Motore compl" o "Motore semicompl"
      // inoltre confronto marca/modello/anno con funzione sopra
      ...
      motoreRichiesta.forEach(rigaRichiesta => {
        ...
        // Cerco nel inventario tutte le righe con lo stesso codice motore (colonna "Veicolo/Tipo Motore (EcoEuro)")
        const matchesInventario = inventarioData.filter(rigaInv => {
          ...
          // Controllo matching marca/modello/anno
